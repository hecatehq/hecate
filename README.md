...
> available, while additional workflow modes, browser typing and visual
> capture, broader Work policy integrations, and sandbox hardening are still evolving
> work. Read
...
flowchart LR
    ...
    subgraph HecateProcess["Local Hecate runtime process"]
        ...
        TaskRuntime["Hecate task runtime<br/>Work policy snapshots · queue · agent_loop<br/>retry/resume"]
        AgentSupervisor["External Agent supervisor<br/>identity trust · ACP sessions · diagnostics"]
...
| **External Agent**   | Supervised local ACP sessions for Codex, Claude Code, Cursor Agent, and Grok Build, including file inputs, passive executable identity measurement, explicit runtime-host approval and start-time revalidation, fresh session handshakes, prompt-time vendor readiness, optional version/auth diagnostics, prompt-first action approvals, cancellation, and Git diff inspect/revert. External agents keep their own accounts/billing.                                                                                                                                                                                |
...
- Richer Work policy integrations and preset workflows.
...
When a Chat uses a Work policy, its tools-on backing Tasks also keep the
policy's frozen approval posture. `require` asks before every otherwise allowed
tool call; `block` refuses actions that would otherwise ask for approval.
Direct turns have no tool calls to approve, and Chat still receives no browser
grant from a Work policy.
...
Standard native Tasks can also select a global **Work policy** in the New Task
panel. Hecate freezes its execution profile, route hints, instructions, tools,
write, network, approval, and browser posture when the Task is created; later
policy edits do not change that Task or its retries. Standalone Tasks do not
activate Project memory, context sources, skills, or Cairnline coordination.
...
Before first use, open **Connections**, review the measured canonical path and
SHA-256, and approve that exact app identity. Discovery and opening Connections
remain passive; **Check**, **New chat**, auth/logout, and deferred embedded
provider launches revalidate approval at process start. Approval detects an
unexpected change but does not certify a publisher, prove the app is safe, or
eliminate the residual same-user path-swap window.
