...
| Hecate Agent chats | Implemented | Hecate Chat can switch between direct model segments and task-backed `agent_loop` segments in one transcript. Consecutive tools-on turns continue the latest terminal run; re-enabling tools after a model segment starts a fresh backing Task. Requires a model known to support tools |
