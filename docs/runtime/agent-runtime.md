...
| `terminal_open`          | Open a long-lived terminal process in the workspace                   | Gated by `shell_exec` or `all_tools` policy (default on); uses `LocalWorkspace.OpenTerminal`, the same static command policy, env sanitisation, workspace root, and optional OS wrapper as one-shot shell calls                                                           |
...
opened. Terminal handles are scoped to the current run. They are kept alive
