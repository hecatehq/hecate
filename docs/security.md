...
- Claude Code adapter setup tokens are long-lived OAuth tokens generated by `claude setup-token` and stored only for launching the local Claude ACP adapter. Hecate does not run Anthropic's OAuth flow itself. This local-only flow is for an operator's own Hecate instance, not for hosted or multi-user Hecate deployments that collect or share users' Claude subscription setup tokens.
- These tokens do not make Claude subscription usage free. Anthropic's [Claude Code authentication docs](https://code.claude.com/docs/en/authentication#generate-a-long-lived-token) say that starting June 15, 2026, Agent SDK and `claude -p` subscription usage draws from a separate monthly Agent SDK credit, and Anthropic's [Agent SDK credit guidance](https://support.claude.com/en/articles/15036540-use-the-claude-agent-sdk-with-your-claude-plan) says that credit is per-user, cannot be pooled, and that shared production automation should use the Claude Developer Platform with an API key.
