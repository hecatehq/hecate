...
just test-codeintel-dogfood # opt-in real-model code-intelligence scorecard
...
The [code-intelligence model dogfood](code-intelligence-dogfood.md) is also
opt-in because it sends fixed repository questions to an operator-configured
tool-capable model and may consume provider quota. It launches through real
Project assignments, writes sanitized local reports below `.dogfood/`, and is
observational by default so stochastic model behavior does not become a normal
CI failure.
