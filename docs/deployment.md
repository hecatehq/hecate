...
Hecate defaults to `127.0.0.1:8765` and enforces same-origin for browser requests; there is no auth layer. The threat model is "trust your own machine" — anyone with local user access to the host has full access to the gateway. If you change `GATEWAY_ADDRESS` to expose Hecate beyond the local machine, put your own auth, firewall, or reverse proxy in front.
...
```yaml
# docker-compose.yml
image: ghcr.io/chicoxyzzy/hecate:0.1.0-alpha.14
```
...
curl -LO https://github.com/chicoxyzzy/hecate/releases/download/v0.1.0-alpha.14/hecate_0.1.0-alpha.14_linux_amd64.tar.gz
tar -xzf hecate_0.1.0-alpha.14_linux_amd64.tar.gz
./hecate
```
...
Available tarballs for `v0.1.0-alpha.14`:

- `hecate_0.1.0-alpha.14_linux_amd64.tar.gz`
- `hecate_0.1.0-alpha.14_linux_arm64.tar.gz`
- `hecate_0.1.0-alpha.14_darwin_amd64.tar.gz`
- `hecate_0.1.0-alpha.14_darwin_arm64.tar.gz`
