...
| Operator shell access                                 | Hecate does not expose an embedded browser/API terminal. Local operators can still open their normal OS terminal from the workspace menu; remote and container operators should use the surrounding infrastructure shell (`ssh`, `docker exec`, `kubectl exec`, provider console, or equivalent) when they need direct host access. Those shells are outside Hecate's approval, audit, and sandbox surfaces. |
...
- Hecate-mediated workspace file operations use the shared WorkspaceFS resolver. This covers native `agent_loop` file/search tools, sandboxed file writes, generated workspace setup, and ACP adapter read/write callbacks. Opt-in ACP terminal RPCs are constrained to the selected workspace and approval-gated before spawn, but still run as trusted External Agent subprocess work. They do not sandbox external-agent subprocess internals.
