...
| Embedded workspace terminal                           | Local-only operator surface. The UI first mints a short-lived, one-use, workspace-bound terminal ticket through the protected local API, then opens a WebSocket terminal that starts the host shell in the selected workspace and runs as the local OS user. It is observable as ordinary workspace changes, but it is intentionally not approval-gated or sandboxed like task-runtime tool calls. |
