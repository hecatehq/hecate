...
| Embedded workspace terminal                           | Operator surface. The UI first mints a short-lived, one-use, workspace-bound terminal ticket through the protected API, then opens a WebSocket terminal that starts the host shell in the selected workspace, or in the runtime working directory when no workspace is selected. Local runtimes require loopback clients and reject forwarded-client headers. Hosted runtimes require runtime identity middleware on the ticket-mint route; the WebSocket consumes only the one-use ticket because browsers cannot attach custom headers during upgrade. It is observable as ordinary workspace changes, but it is intentionally not approval-gated or sandboxed like task-runtime tool calls. Set `HECATE_EMBEDDED_TERMINAL=false` to remove this capability and make the terminal routes return 404. |
...
The embedded workspace terminal is not an agent tool. Agents must not receive or
reuse terminal session tickets. Hecate-owned command execution should go through
the governed task-runtime tools, where WorkspaceFS, ProcessRunner, GitRunner,
approvals, timeouts, output caps, and sandbox policies can apply.
If a hosted or shared deployment requires every command to pass through those
governed surfaces, disable the embedded terminal and rely on the surrounding
container, VM, or dedicated OS-user boundary for any remaining operator shell
access.
