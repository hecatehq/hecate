...
- **Reset local data does not relaunch the app.** Settings → Maintenance → Danger zone calls `POST /hecate/v1/system/reset-data`, closes live external-agent chat sessions, and removes Hecate-owned projects, chats, tasks, provider setup, policy rules, and saved external-agent grants in place. Workspace files and external CLI auth files stay on disk.
