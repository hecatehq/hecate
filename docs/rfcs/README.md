...
| [Local models — Hecate-managed llama.cpp runtime](local-models-llamacpp.md) | Accepted / in progress. Bundled `llama-server`, Hugging Face installs, a gateway-internal proxy, and `/v1/models` surfacing exist on this feature branch. Linux / Windows bundles remain out of scope.                                                                            | Revalidate against current master, then decide whether to split runtime, UI, and desktop bundling before merge.    |
