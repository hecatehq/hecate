...
| [Local models — Hecate-managed llama.cpp runtime](local-models-llamacpp.md) | Design notes for a bundled `llama-server` sidecar, HuggingFace-sourced curated catalog with paste-URL escape hatch, single-child restart-on-switch lifecycle, and a gateway-internal proxy that surfaces installed models through `/v1/models`. v1 is macOS arm64 desktop only; Linux / Windows and HF browse reserved for follow-ups. Not implemented. |
