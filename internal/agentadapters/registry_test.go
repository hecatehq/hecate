...
func TestBuiltInsIncludeInitialExternalAgents(t *testing.T) {
	...
	if got := found["codex"]; got.SupportedRange != ">=0.1.0-alpha.9" {
	...
	if got := found["claude_code"]; got.SupportedRange != ">=0.1.0-alpha.9" {
...
func TestStatusForAdapterWarnsOnOldGoAdapterRelease(t *testing.T) {
	dir := t.TempDir()
	adapterBinary := writeFakeBinary(t, dir, "codex-acp-adapter", "codex-acp-adapter 0.1.0-alpha.8")
	agent := writeFakeBinary(t, dir, "codex", "codex 9.8.7")
	adapter := Adapter{
		ID:             "codex",
		Name:           "Codex",
		Command:        "codex-acp-adapter",
		SupportedRange: ">=0.1.0-alpha.9",
		AgentVersion: VersionProbe{
			Command: "codex",
			Args:    []string{"--version"},
		},
	...
	if status.AdapterVersion != "0.1.0-alpha.8" {
		t.Fatalf("status.AdapterVersion = %q, want 0.1.0-alpha.8", status.AdapterVersion)
	}
	if !status.VersionOutsideRange {
		t.Fatal("status.VersionOutsideRange = false, want warning for pre-alpha.9 adapter")
	}
}
