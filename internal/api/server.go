...
func NewServer(logger *slog.Logger, handler *Handler) http.Handler {
	if err := handler.StartTaskRuntime(context.Background()); err != nil && logger != nil {
...
func registerHecateProjectRoutes(mux *http.ServeMux, handler *Handler) {
	...
	mutateProject := handler.projectMutationHandler
	mux.HandleFunc("PATCH /hecate/v1/projects/{id}", mutateProject(handler.HandleUpdateProject))
	mux.HandleFunc("DELETE /hecate/v1/projects/{id}", handler.HandleDeleteProject)
	mux.HandleFunc("POST /hecate/v1/projects/{id}/roots", mutateProject(handler.HandleCreateProjectRoot))
	mux.HandleFunc("POST /hecate/v1/projects/{id}/roots/discover", mutateProject(handler.HandleDiscoverProjectRoots))
	mux.HandleFunc("POST /hecate/v1/projects/{id}/roots/worktrees", mutateProject(handler.HandleCreateProjectWorktreeRoot))
	mux.HandleFunc("PATCH /hecate/v1/projects/{id}/roots/{root_id}", mutateProject(handler.HandleUpdateProjectRoot))
	mux.HandleFunc("DELETE /hecate/v1/projects/{id}/roots/{root_id}", mutateProject(handler.HandleDeleteProjectRoot))
	mux.HandleFunc("POST /hecate/v1/projects/{id}/context-sources", mutateProject(handler.HandleCreateProjectContextSource))
	mux.HandleFunc("POST /hecate/v1/projects/{id}/context-sources/discover", mutateProject(handler.HandleDiscoverProjectContextSources))
	mux.HandleFunc("PATCH /hecate/v1/projects/{id}/context-sources/{source_id}", mutateProject(handler.HandleUpdateProjectContextSource))
	mux.HandleFunc("DELETE /hecate/v1/projects/{id}/context-sources/{source_id}", mutateProject(handler.HandleDeleteProjectContextSource))
	mux.HandleFunc("GET /hecate/v1/projects/{id}/skills", handler.HandleProjectSkills)
	mux.HandleFunc("POST /hecate/v1/projects/{id}/skills/discover", mutateProject(handler.HandleDiscoverProjectSkills))
	mux.HandleFunc("PATCH /hecate/v1/projects/{id}/skills/{skill_id}", mutateProject(handler.HandleUpdateProjectSkill))
	mux.HandleFunc("GET /hecate/v1/projects/{id}/memory", handler.HandleProjectMemoryEntries)
	mux.HandleFunc("POST /hecate/v1/projects/{id}/memory", mutateProject(handler.HandleCreateProjectMemoryEntry))
	mux.HandleFunc("GET /hecate/v1/projects/{id}/memory/candidates", handler.HandleProjectMemoryCandidates)
	mux.HandleFunc("POST /hecate/v1/projects/{id}/memory/candidates", mutateProject(handler.HandleCreateProjectMemoryCandidate))
	mux.HandleFunc("POST /hecate/v1/projects/{id}/memory/candidates/{candidate_id}/promote", mutateProject(handler.HandlePromoteProjectMemoryCandidate))
	mux.HandleFunc("POST /hecate/v1/projects/{id}/memory/candidates/{candidate_id}/reject", mutateProject(handler.HandleRejectProjectMemoryCandidate))
	mux.HandleFunc("PATCH /hecate/v1/projects/{id}/memory/{memory_id}", mutateProject(handler.HandleUpdateProjectMemoryEntry))
	mux.HandleFunc("DELETE /hecate/v1/projects/{id}/memory/{memory_id}", mutateProject(handler.HandleDeleteProjectMemoryEntry))
	...
	mux.HandleFunc("POST /hecate/v1/projects/{id}/roles", mutateProject(handler.HandleCreateProjectWorkRole))
	mux.HandleFunc("PATCH /hecate/v1/projects/{id}/roles/{role_id}", mutateProject(handler.HandleUpdateProjectWorkRole))
	mux.HandleFunc("DELETE /hecate/v1/projects/{id}/roles/{role_id}", mutateProject(handler.HandleDeleteProjectWorkRole))
	mux.HandleFunc("GET /hecate/v1/projects/{id}/work-items", handler.HandleProjectWorkItems)
	mux.HandleFunc("POST /hecate/v1/projects/{id}/work-items", mutateProject(handler.HandleCreateProjectWorkItem))
	...
	mux.HandleFunc("PATCH /hecate/v1/projects/{id}/work-items/{work_item_id}", mutateProject(handler.HandleUpdateProjectWorkItem))
	mux.HandleFunc("DELETE /hecate/v1/projects/{id}/work-items/{work_item_id}", mutateProject(handler.HandleDeleteProjectWorkItem))
	mux.HandleFunc("GET /hecate/v1/projects/{id}/work-items/{work_item_id}/assignments", handler.HandleProjectWorkAssignments)
	mux.HandleFunc("POST /hecate/v1/projects/{id}/work-items/{work_item_id}/assignments", mutateProject(handler.HandleCreateProjectWorkAssignment))
	mux.HandleFunc("PATCH /hecate/v1/projects/{id}/work-items/{work_item_id}/assignments/{assignment_id}", mutateProject(handler.HandleUpdateProjectWorkAssignment))
	mux.HandleFunc("DELETE /hecate/v1/projects/{id}/work-items/{work_item_id}/assignments/{assignment_id}", mutateProject(handler.HandleDeleteProjectWorkAssignment))
	...
	mux.HandleFunc("POST /hecate/v1/projects/{id}/work-items/{work_item_id}/assignments/{assignment_id}/start", mutateProject(handler.HandleStartProjectWorkAssignment))
	...
	mux.HandleFunc("POST /hecate/v1/projects/{id}/work-items/{work_item_id}/artifacts", mutateProject(handler.HandleCreateProjectWorkArtifact))
	...
	mux.HandleFunc("POST /hecate/v1/projects/{id}/work-items/{work_item_id}/handoffs", mutateProject(handler.HandleCreateProjectHandoff))
	mux.HandleFunc("PATCH /hecate/v1/projects/{id}/work-items/{work_item_id}/handoffs/{handoff_id}", mutateProject(handler.HandleUpdateProjectHandoff))
	mux.HandleFunc("POST /hecate/v1/projects/{id}/work-items/{work_item_id}/handoffs/{handoff_id}/status", mutateProject(handler.HandleUpdateProjectHandoffStatus))
	mux.HandleFunc("POST /hecate/v1/projects/{id}/work-items/{work_item_id}/handoffs/{handoff_id}/accept-with-follow-up", mutateProject(handler.HandleAcceptProjectHandoffWithFollowUp))
	mux.HandleFunc("DELETE /hecate/v1/projects/{id}/work-items/{work_item_id}/handoffs/{handoff_id}", mutateProject(handler.HandleDeleteProjectHandoff))
}
...
func registerHecateAgentRoutes(mux *http.ServeMux, handler *Handler) {
	...
	mux.HandleFunc("PUT /hecate/v1/agent-adapters/{id}/executable-trust", handler.HandleApproveAgentAdapterExecutable)
	mux.HandleFunc("DELETE /hecate/v1/agent-adapters/{id}/executable-trust", handler.HandleRevokeAgentAdapterExecutable)
	...
	mux.HandleFunc("POST /hecate/v1/chat/sessions/{id}/attachments", handler.HandleCreateChatAttachment)
	mux.HandleFunc("DELETE /hecate/v1/chat/sessions/{id}/attachments/{attachment_id}", handler.HandleDeleteChatAttachment)
	mux.HandleFunc("GET /hecate/v1/chat/sessions/{id}/attachments/{attachment_id}/content", handler.HandleChatAttachmentContent)
