...
func NewServer(logger *slog.Logger, handler *Handler) http.Handler {
	...
	return Chain(
		...
		RequestIDMiddleware,
		CloudRuntimeIdentityMiddleware(handler.config.Server.CloudRuntimeMode, handler.config.Server.CloudRuntimeSecret),
		OTelHTTPSpanMiddleware,
		SameOriginMiddlewareWithAllowedOrigins(handler.config.Server.AllowedOrigins),
		CloudRuntimeLocalEndpointGuardMiddleware(handler.config.Server.CloudRuntimeMode),
...
func registerHecateOperationsRoutes(mux *http.ServeMux, handler *Handler) {
	...
	mux.HandleFunc("POST /hecate/v1/terminal/sessions", handler.HandleCreateTerminalSession)
	mux.HandleFunc("GET /hecate/v1/terminal", handler.HandleTerminal)
