	...
	for _, item := range items {
		out = append(out, projectwork.Assignment{
			...
			ExecutionRef: projectwork.NormalizeAssignmentExecutionRef(projectwork.AssignmentExecutionRef{
				Kind:                 item.ExecutionRef.Kind,
				TaskID:               item.ExecutionRef.TaskID,
				RunID:                item.ExecutionRef.RunID,
				ChatSessionID:        item.ExecutionRef.SessionID,
				ContextSnapshotID:    item.ContextSnapshotID,
				TraceID:              item.ExecutionRef.TraceID,
				PendingApprovalCount: item.ExecutionRef.PendingApprovals,
			}),
