...
func waitProcessExitWithoutReaping(ctx context.Context, cmd *exec.Cmd) error {
	...
	if _, err := unix.Kevent(kqueue, []unix.Kevent_t{change}, nil, nil); errors.Is(err, unix.ESRCH) {
		// A very short-lived child can exit before EVFILT_PROC is
		// registered. It is still unreaped, so its PID cannot have been
		// reused; ESRCH is therefore an observed exit, not an ambiguity.
		return nil
	} else if err != nil {
		return err
	}
