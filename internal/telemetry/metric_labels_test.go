...
func TestMetricClosedSetNormalizersCollapseUnknownValues(t *testing.T) {
	...
	cases := []struct {
		...
		{"queue backend sqlite", "sqlite", NormalizeQueueBackend, "sqlite"},
		{"queue backend postgres", "Postgres", NormalizeQueueBackend, "postgres"},
		{"queue backend unknown", "redis", NormalizeQueueBackend, MetricLabelOther},
