...
- **Auto-update emits `latest.json` per release.** `_tauri-shared.yml` passes `TAURI_UPDATER_PRIVATE_KEY` + `TAURI_UPDATER_PRIVATE_KEY_PASSWORD` to tauri-action (gated on `inputs.tagName != ''` — same caller-side / called-side protection model as the Apple secrets). tauri-action signs each platform bundle and uploads `latest.json` to the GitHub Release. Existing installs check this manifest on launch and surface a banner when a newer version is published. Maintainer-side keypair custody and rotation: [`../../docs/desktop-updater-signing.md`](../../docs/desktop-updater-signing.md). If the secrets ever go missing, tauri-action skips signing and no `latest.json` ships — existing installs stop seeing updates until the next signed release.
...
- `latest.json` is attached as a release asset (the auto-updater manifest). Missing means the signing step skipped — check that `TAURI_UPDATER_*` secrets are still configured in repo settings.
