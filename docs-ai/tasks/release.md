...
- **Bundled External Agent versions are deliberate.** The Dockerfiles pin the top-level npm package versions for Codex, Claude Code, their ACP adapters, and Grok Build. Cursor Agent comes from Cursor's official installer; because that script has no version flag, the Dockerfiles pin the installer SHA-256 and fail the build when Cursor rotates it. Review the new script, confirm the hardcoded Cursor package it installs, then update both Dockerfiles together.
