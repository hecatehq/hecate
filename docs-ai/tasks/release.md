...
4. `tauri / publish updater manifest to website` (`needs: publish-updater-manifest`) — commits `latest.json` to `website/public/releases/alpha/` on master, then polls `https://hecate.sh/releases/alpha/latest.json` until the new version is live. Triggers the website workflow as a side effect; total time including the website rebuild + Pages redeploy is typically 2–5 min, capped at 10 min before the job fails.
