...
- **Webview media access still needs native privacy metadata.** Chat dictation
  ...
  `NSMicrophoneUsageDescription` in `src-tauri/Info.plist`; Tauri merges that
  file into the bundle. Keep the purpose string user-facing, and verify the
  generated app plist when packaging behavior changes.
