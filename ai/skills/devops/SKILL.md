...
- **Schema migrations.** Which storage tiers are affected? Memory is rebuilt on boot (fine). SQLite needs a forward-compatible migration and roll-forward considerations. The retention worker subsystems (`traces`, `budget`, `audit`, `provider_history`, `turn_events`, `agent_chat_approvals`) must keep mirroring.
