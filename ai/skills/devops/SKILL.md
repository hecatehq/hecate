...
- **Schema migrations.** Which storage tiers are affected? Memory is rebuilt on boot (fine). Sqlite needs a forward-compat migration. The retention worker subsystems (`traces`, `budget`, `audit`, `cache`, `turn_events`) must keep mirroring.
