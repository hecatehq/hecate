...
- **External Agent executable trust closes at process start.** Passive adapter discovery may resolve and hash candidates but must never execute them. `internal/agentadapters` owns the provider-neutral `unavailable` / `unapproved` / `approved` / `changed` state machine, runtime-host-plus-adapter memory/SQLite/Postgres decisions, compare-and-swap approval, and final launch permits. Gate probes, version/auth/help/model diagnostics, authenticate/logout, direct ACP peers, and every embedded provider process, including deferred prompt launches; opening Chats or Connections must not auto-probe. Preserve `launcher_only` as wrapper-only evidence, keep publisher evidence `unavailable` until it is genuinely verified, and never call approval a malware or safety verdict. Hold the permit through child start, while documenting that path-based OS exec retains a narrow same-user swap window. Revoke blocks future launches but does not promise to kill an existing direct peer. Approve/revoke are remote-safe after trusted runtime identity; managed authenticate remains local-only.
...
- **Agent Preset launch posture must reach execution.** `taskapp` resolves an explicit `agent_preset_id` only for a standard standalone native `agent_loop`, validates a `hecate_task` / `any` surface, and freezes the preset at Task creation; `projectworkapp` separately resolves presets and coordinated context for assignments. Explicit provider/model selections override only the corresponding hints. Never pair a preset model hint with an explicitly different provider, and reject an effective provider without a model at creation. The preset execution profile is authoritative, preset instructions precede optional per-Task instructions, and a tools-disabled standalone create rejects MCP servers. Standalone Task resolution must not activate Cairnline state, project memory, context sources, or skills. Start, Schedule dispatch, Retry, Resume, Continue, and retry-from-model-call consume the stored snapshot and never re-resolve the preset. Work-policy-backed native Tasks snapshot the preset id, `tools_enabled`, approval posture, and browser grants/origins, map `writes_allowed` to the Task's read-only sandbox flag, and map `network_allowed` to the Task network flag. Hecate Chat stores its own narrow immutable snapshot and copies its explicit approval posture only to tools-on backing Tasks; legacy Chat snapshots without that field retain their prior behavior, and Chat never acquires browser grants. The approval snapshot is an additive mid-loop policy: `require` gates every otherwise-permitted tool call from the exact advertised catalog; `block` denies only calls that the runtime-wide, mandatory browser, or per-MCP-server policy would otherwise send for approval; `inherit` and `allow` add no gate and never weaken those stricter policies. Hard workflow, tools, sandbox, network, browser-capability/runtime, and MCP-block denials run before approval policy. The snapshot never changes pre-execution gates and does not apply to External Agents, QA, non-preset Tasks, or a Task with an absent snapshot. An explicit tools-disabled snapshot produces an empty native/MCP catalog, starts no MCP host, and fails closed on every unexpected tool call. Preset-backed native HTTP/search tools are not advertised and fail closed when the snapshot disables network. Read-only tasks omit and fail closed on broad shell, Git, file-write, and interactive-terminal surfaces while retaining structured inspection and proposal-only edits. Structured Git inspection must stay passive: use GitRunner's immutable read-only/offline metadata view, preserve safe platform settings such as `core.longpaths`, resolve nested workspaces against the repository top-level while scoping paths and results back to the workspace, disable optional index locks, lazy fetch, fsmonitor helpers, external diff/text conversion, and submodule recursion, resolve effective conversion attributes through bounded NUL-safe Git input/output, and fail closed when a scoped path has an effective or ambiguous content-conversion filter. Do not infer preset tool, approval, browser, or native-network policy from absent snapshots or zero-valued sandbox flags on non-preset Tasks. Cairnline coordination intent never grants runtime permission.
...
- **Workspace review is evidence; discard is separate authority.** `GET workspace-diff` returns fixed staged, working-tree, and untracked read-only layers with opaque per-layer entry ids. Snapshot effective external Git excludes before inventory; surface intent-to-add, assume-unchanged, skip-worktree, and unmerged state as bounded issues instead of hiding it. Inline bodies share a 4 MiB response budget and untracked text is capped at 256 KiB. Open untracked previews only through stable pinned-root no-follow WorkspaceFS reads: refuse symlinks, special files, unstable identity, hardlinks, Unix cross-device entries, and tracked or untracked nested repositories; use Windows `os.Root` confinement. Classify the most-specific mount before sensitive reads and recheck the opened handle; network, FUSE, and unknown filesystem classes fail closed, including nested mounts. Every live review/discard response is `private, no-store`; remote callers may receive raw bounded untracked text, but paths, content, and revisions must never enter logs or telemetry. Only the root-bound `workspace-sha256:` value in `data.discard.revision` from one complete exact raw scoped **unstaged tracked** `DiffSnapshot` (index → worktree) authorizes discard; layered previews, opaque ids, the legacy top-level patch-only `sha256:` revision, trimmed output, stats, and historical diffs never do. Staged changes remain reviewable but, together with hidden index flags, conflicts, incomplete/truncated working previews, and Gitlinks, close discard. Never delete untracked files. Accept one strict known-field discard JSON object up to 1 MiB. Close and drain the owning session lifecycle, acquire the shared process-local `workspacecoord` exclusive lease for the canonical workspace, reread the session, scan bounded active/non-terminal projections of durable task runs and chats on overlapping roots, and recapture the exact snapshot. Apply the inventory ceiling only after terminal/inactive history is excluded, and preserve unknown non-terminal task statuses in the fail-closed scan. Equality and ancestor/descendant roots overlap; siblings do not. Task provisioning/start admission and execution, External Agent turns and each live ACP terminal, native agent-loop terminals, task-patch apply/revert, and operator terminals hold writer leases from that registry for their complete mutation window. Before mutation, switch to a bounded server-owned context; pin and recheck the reviewed workspace identity, reserve Git's real index lock through handle-relative cleanup, recheck visibility, staged state, and the reviewed baseline, and run a non-mutating reverse-apply preflight. Only preflight failures are clean `409` conflicts. Treat failure of the real apply as typed `outcome_unknown`, surface post-apply cleanup failure without claiming completion unless `outcome=applied`, and suppress further discard until the operator refreshes and inspects Git. The registry is process-local and not distributed; retain the durable-owner scan, root checks, index reservation, preflight, and conditional apply as independent protections.
- **Workspace review filesystem claims stay platform-scoped.** The nested-mount fail-closed guarantee above applies to recognized Linux/macOS mount inventories and canonical Git metadata/object trees. Windows proves a fixed/RAM drive and stable preview handles, but nested Git junction/reparse traversal remains a documented host-policy boundary. Initial bounded Git discovery and OS symlink/root metadata resolution happen before final canonical-path checks. Do not claim those residual operations or a failing physical disk are synchronously cancellable.
...
| Cut a release tag                                                           | `bun scripts/release.ts vX.Y.Z --notes docs/releases/vX.Y.Z.md` — validates curated notes and the worktree, runs the snapshot, stamps Tauri versions, tags, and pushes. Full procedure: [`docs-ai/tasks/release.md`](docs-ai/tasks/release.md) |
