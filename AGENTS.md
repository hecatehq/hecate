...
- **Hecate Chat workspace posture is session-owned and task-enforced.** Hecate-owned sessions persist `persistent`, `ephemeral`, or `in_place`; backing tasks must receive the same value. Omitted API values retain the legacy `in_place` contract, while the operator UI explicitly chooses a safe managed-workspace default or the linked Cairnline Project default. A session may change posture only before any backing task exists, and mode mutation must hold the same exclusive live-run admission used by turns so concurrent creation cannot snapshot a different value. Managed runs must rebind the session, message evidence, and context packet to the generated run workspace so review never inspects the untouched source path. External Agent sessions always remain `in_place`; ACP adapters own their selected workspace directly. Do not turn Cairnline coordination defaults into runtime permission or duplicate them in a Hecate Projects store.
